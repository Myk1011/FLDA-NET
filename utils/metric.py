...
class Evaluator(object):
    def __init__(self, num_class, log_name):
    ...
    def Kappa(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        po = tp.sum() / self.confusion_matrix.sum()
        pe = (self.confusion_matrix.sum(axis=0) * self.confusion_matrix.sum(axis=1)).sum() / self.confusion_matrix.sum() ** 2
        Kappa = (po - pe) / (1 - pe)
        return Kappa
    ...
    def _generate_matrix(self, gt_image, pre_image):
        if 'vaihingen' in self.log_name:
            mask = (gt_image >= 0) & (gt_image < self.num_class - 1)
        elif 'potsdam' in self.log_name:
            mask = (gt_image >= 0) & (gt_image < self.num_class - 1)
        else:
...
if __name__ == '__main__':
    ...
    eval = Evaluator(num_class=3, log_name='potsdam')
